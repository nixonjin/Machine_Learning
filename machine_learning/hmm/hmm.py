...
class HMM(object):
    ...
    def decoding(self, word_list,word2id,tag2id):
        ...
        # 其中viterbi[i, j]表示标注序列的第i个标注为j的最大概率
        seq_len = len(word_list)
        viterbi = torch.zeros(seq_len,self.N)
        # backpointer是跟viterbi一样大小的矩阵
        # backpointer[i, j]存储的是 标注序列的第i个标注为j时，第i-1个标注的id
        # 等解码的时候，我们用backpointer进行回溯，以求出最优路径
        backpointer = torch.zeros(seq_len, self.N).long()
        ...
        for step in range(1, seq_len):
            ...
            for tag_id in range(len(tag2id)):
                # max_id是转移到当前状态的tag_id概率最大的前一个状态id
                max_prob, max_id = torch.max(viterbi[step-1,:]+A[:,tag_id],dim=0)
                viterbi[step,tag_id] = max_prob + bt[tag_id]
                backpointer[step,tag_id] = max_id
        
        # 终止， t=seq_len即viterbi[seq_len,:]中的最大概率，就是最有路径的概率
        ...
        for back_step in range(seq_len-1,0,-1):
            best_path_pointer = backpointer[back_step, best_path_pointer]
            best_path_pointer = best_path_pointer.item()
